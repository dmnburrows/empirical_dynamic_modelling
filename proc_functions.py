...
#=======================================================================================     
def plot_random_convergence_profiles(conv_arr, n=10, seed=None):
#=======================================================================================     

    """
    Randomly selects `n` convergence traces from conv_arr and plots them
    across interval lengths. The selected traces are sorted by their final
    value in descending order.

    Parameters
    ----------
    conv_arr : np.ndarray
        2D array of shape (n_intervals, n_elements), where each row is a flattened
        2D CCM matrix at a different library size or interval length.
    n : int
    ...
    num_elements = conv_arr.shape[1]
    rand_idx = np.random.choice(num_elements, size=n, replace=False)

    # Extract those columns (i.e. convergence traces) and sort by final value
    subset = conv_arr[:, rand_idx]
    subset_sorted = subset[::-1]


    # Plot
    plt.figure(figsize=(7, 4))
    for i in range(n):
        plt.plot(subset_sorted[:, i], label=f'Index {rand_idx[i]}', alpha=0.7)
    
    plt.xlabel("Interval index (increasing length)")
    plt.ylabel("Cross map skill (ρ)")
    plt.title("Randomly selected convergence profiles")
    plt.grid(True)
    plt.tight_layout()
...
#=================================================================================
def conv_check(curr_l=None, n_int=3, int_l = [1.2, 1.4, 1.8], ratio_val=-0.2):
#=================================================================================
    """
    ...
    conv_arr.append(np.ravel(np.load(np.asarray(curr_l)[match][0])))
    ...
    match = pd.Series(curr_l).str.contains(r'run-01_(trace|sz_trace|pre_trace)_CCMxmap')
    og= np.load(np.asarray(curr_l)[match][0])
